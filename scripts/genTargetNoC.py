'''
Generate NoC code for target chiplet network
'''
DATA_WIDTH = 64
NETWORK_WIDTH = 4
NETWORK_HEIGHT = 4

file_name = r"F:\git\simMTR\rtl\TargetNoC.txt"
...
router #(
    ...
    .local_data_i          (local_data_i_''' + str(j) + '''_''' + str(i) + '''),
    .local_valid_i         (local_valid_i_''' + str(j) + '''_''' + str(i) + '''),
    .local_ready_o         (local_ready_o_''' + str(j) + '''_''' + str(i) + '''),
    .local_data_o          (local_data_o_''' + str(j) + '''_''' + str(i) + '''),
    .local_valid_o         (local_valid_o_''' + str(j) + '''_''' + str(i) + '''),
    .local_ready_i         (local_ready_i_''' + str(j) + '''_''' + str(i) + '''),
    .bound_data_i          ('''+bound_in_data+'''),
    .bound_valid_i         ('''+bound_in_valid+'''),
    .bound_ready_o         ('''+bound_in_ready+'''),
    .bound_data_o          ('''+bound_out_data+'''),
    .bound_valid_o         ('''+bound_out_valid+'''),
    .bound_ready_i         ('''+bound_out_ready+''')
);\n'''
            inst_str += router_txt
    return inst_str
...
module TargetNoC #(
    ...
    for i in range(h):
        for j in range(w):
            local_temp_txt = '''
    //router''' + str(i*w+j) + '''
    input       wire        [`DW-1:0]           local_data_i_''' + str(j) + '''_''' + str(i) + ''',
    input       wire                            local_valid_i_''' + str(j) + '''_''' + str(i) + ''',
    output      wire                            local_ready_o_''' + str(j) + '''_''' + str(i) + ''',

    output      wire        [`DW-1:0]           local_data_o_''' + str(j) + '''_''' + str(i) + ''',
    output      wire                            local_valid_o_''' + str(j) + '''_''' + str(i) + ''',
    input       wire                            local_ready_i_''' + str(j) + '''_''' + str(i)
    ...
    input       wire                            bound_ready_i_''' + str(j) + '''_''' + str(i)                      
            ...
            port_str += local_temp_txt + "," + bound_temp_txt
