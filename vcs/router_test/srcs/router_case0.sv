...
class drv1_seq extends uvm_sequence #(cibd_transaction);
    ...
    virtual task body();
        repeat(`PKT_NUM) begin
            `uvm_do_with(trans,{trans.rtdrid_x >= `LOCAL_ID_X;})
...
class drv0_seq extends uvm_sequence #(cibd_transaction);
    ...
    virtual task body();
        repeat(`PKT_NUM) begin
            `uvm_do_with(trans,{trans.rtdrid_y >= `LOCAL_ID_Y; 
                                trans.rtdrid_x == `LOCAL_ID_X;})
...
class drv4_seq extends uvm_sequence #(cibd_transaction);
    ...
    virtual task body();
        repeat(`PKT_NUM) begin
            `uvm_do_with(trans,{trans.outbound == 1'b1;}
                )
...
//sequence for router bound input port
class drv5_seq extends uvm_sequence #(cibd_transaction);
    cibd_transaction trans;
    `uvm_object_utils(drv5_seq)

    function new(string name= "drv5_seq");
        super.new(name);
    endfunction 
    
    virtual task body();
        repeat(`PKT_NUM) begin
            `uvm_do_with(trans,{trans.outbound == 1'b0;}) 
            `uvm_info("drv5_seq", "send one transaction", UVM_LOW)
        end
    endtask
endclass
...
class router_case0_vseq extends uvm_sequence;
    ...
    virtual task body();
        ...
        fork
            `uvm_do_on(seq0, p_sequencer.p_sqr_n)
            `uvm_do_on(seq1, p_sequencer.p_sqr_w)
            `uvm_do_on(seq2, p_sequencer.p_sqr_s)
            `uvm_do_on(seq3, p_sequencer.p_sqr_e)
            `uvm_do_on(seq4, p_sequencer.p_sqr_l)
            `uvm_do_on(seq5, p_sequencer.p_sqr_b)
        join 
        #10000;
        if(starting_phase != null) 
