class Agent;
    cibd_agent i_agt,o_agt;
endclass

class FIFO;
    uvm_tlm_analysis_fifo #(cibd_transaction)
    drv_mdl_fifo,mdl_sb_fifo,mon_sb_fifo;
endclass

class mtr_env extends uvm_env;
    `uvm_component_utils(mtr_env)
    ...
    function new(string name = "mtr_env",uvm_component parent = null);
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        agent = new[4*`NOC_WIDTH*`NOC_HEIGHT];
        fifo = new[4*`NOC_WIDTH*`NOC_HEIGHT];

        foreach(agent[i]) begin
            agent[i] = new();
            agent[i].i_agt = cibd_agent::type_id::create($sformatf("agent[%0d].i_agt",i),this);
            agent[i].i_agt.is_active = UVM_ACTIVE;
            agent[i].o_agt = cibd_agent::type_id::create($sformatf("agent[%0d].o_agt",i),this);
            agent[i].o_agt.is_active = UVM_PASSIVE;
        end

        `uvm_info("mtr_env","all i_agts are built",UVM_LOW)
        `uvm_info("mtr_env","all o_agts are built",UVM_LOW)
        
        mdl = mtr_model::type_id::create("mdl",this);
        sb = mtr_scoreboard::type_id::create("sb",this);

        `uvm_info("mtr_env","mdl is built",UVM_LOW)
        `uvm_info("mtr_env","sb is built",UVM_LOW)

        foreach(fifo[i]) begin
            fifo[i] = new();
            fifo[i].drv_mdl_fifo = new($sformatf("fifo[%0d].drv_mdl_fifo",i),this);
            fifo[i].mdl_sb_fifo = new($sformatf("fifo[%0d].mdl_sb_fifo",i),this);
            fifo[i].mon_sb_fifo = new($sformatf("fifo[%0d].mon_sb_fifo",i),this);
        end
    ...
    virtual function void connect_phase(uvm_phase phase);
        ...
        for(int i = 0; i < 4*`NOC_WIDTH*`NOC_HEIGHT; i++)
        begin
            agent[i].i_agt.ap.connect(fifo[i].drv_mdl_fifo.analysis_export); //TLM
            mdl.mport[i].bp.connect(fifo[i].drv_mdl_fifo.blocking_get_export); //TLM

            mdl.mport[i].ap.connect(fifo[i].mdl_sb_fifo.analysis_export); //TLM
            sb.sport[i].exp_bp.connect(fifo[i].mdl_sb_fifo.blocking_get_export); //TLM
            
            agent[i].o_agt.ap.connect(fifo[i].mon_sb_fifo.analysis_export); //TLM
            sb.sport[i].act_bp.connect(fifo[i].mon_sb_fifo.blocking_get_export); //TLM
        end
        ...
        this.sb.set_report_id_file("mtr_scoreboard",report_log);
        ...
        this.sb.set_report_id_action("mtr_scoreboard",UVM_DISPLAY | UVM_LOG);
    endfunction
